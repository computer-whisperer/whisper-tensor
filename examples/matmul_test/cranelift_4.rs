...
impl CraneliftBuilder {
    ...
    fn define_matmul_func(module: &mut JITModule, m: i64, n: i64, k: i64) -> (cranelift_module::FuncId, String) {
        ...
        // Create accumulator variables c[MR][NR] and init to 0
        let mut c_vars: Vec<Variable> = Vec::new();
        for mi in 0..MR {
            for nj in 0..NR {
        ...
        // Load b[NR] contiguous from Bp row p: idx_bp = p*NB + (jj0+nj)
        ...
        for nj in 0..NR {
            ...
            let jj_nj = b.ins().iadd(jj0, nj_val);
        ...
        // After K: store microtile directly to C with bounds
        b.switch_to_block(lmk_p_after);
        for mi in 0..MR {
            for nj in 0..NR {
                // Global coordinates I=i0_base+ii0+mi, J=j0_base+jj0+nj
                ...
                let jj0_plus_nj = b.ins().iadd(jj0, nj_v);
        ...
        let func_id = module
            .declare_function("matmul_f32_cl4", Linkage::Local, &ctx.func.signature)
...
impl MatmulImpl for CraneliftImpl {
    fn name(&self) -> &'static str { "cranelift_4" }
