...
fn main() {
    ...
    let mut runtime = RuntimeModel::load_onnx(&onnx_data, RuntimeBackend::Eval(EvalBackend::NDArray), Default::default()).unwrap();
    ...
    let input_tensor = NumericTensor::from_vec1(input).unsqueeze(0, &EvalBackend::NDArray).unwrap();
    ...
    let output_values: Vec<u32> = output.squeeze(0, &EvalBackend::NDArray).unwrap().try_into().unwrap();
