            ...
            if f32_gb > 120.0 {
                ...
                for g in result.graph.groups() {
                    if let ScalarOp::Literal(scalar) = &g.op {
                        ...
                        for i in 0..g.count {
                            shared[(g.base_id.0 + i) as usize] = val;
                        }
                    }
                }
                ...
                for (name, (_dtype, _shape_dims)) in &input_info {
                    ...
                    if let Some(id) = tensors_by_name.get(name) {
                        ...
                        if let Some(tam) = full_result.tensor_map.get(id) {
                            let f32_t = tensor
                                .cast(whisper_tensor::dtype::DType::F32, &mut backend_v4)
                                .unwrap();
                            let flat = f32_t.flatten().unwrap();
                            let nd = flat.to_ndarray().unwrap();
                            let v: Vec<f32> = nd.try_into().unwrap();
                                ...
                                let val = match &group.op {
                                    ...
                                    ScalarOp::Binary { op, .. } => {
                                        ...
                                        match op {
                                            ...
                                            Mul => a * b,
                                            Div => {
                                                if b == 0.0 && panic_group.is_none() {
                                                    panic_group = Some((pi, li, gi, "div by zero"));
                                                }
                                                a / b
                                            }
                                            Max => a.max(b),
                                    ...
                                    ScalarOp::Select { .. } => {
                                        ...
                                        if c != 0.0 {
                                            sbuf[group.inputs[1].resolve(i, 0).0 as usize]
                                        } else {
                                            sbuf[group.inputs[2].resolve(i, 0).0 as usize]
                                        }
                                    }
                                    ScalarOp::ReduceSum {
                                        reduce_count,
                                        ...
                                        for k in 0..*reduce_count {
                                            let src =
                                                (base.0 as i64 + k as i64 * reduce_stride) as usize;
                                            acc += sbuf[src];
                                        }
                                        acc
                                    }
                ...
                let t0 = Instant::now();
                let milli_outputs =
                    whisper_tensor::compiler::interpret_milli_graph(&milli_graph, &milli_inputs2)
                        .unwrap();
                eprintln!("    Milli: {:.1}s", t0.elapsed().as_secs_f64());

                let reverse_map: HashMap<GlobalId, GlobalId> = milli_graph
                    .output_map
                    .as_ref()
                    .map(|m| m.iter().map(|(&i, &e)| (e, i)).collect())
                    .unwrap_or_default();
                ...
                if max_err < 1e-2 {
                    println!("    V4B EVAL: PASS");
                } else {
                    println!("    V4B EVAL: MISMATCH");
                }
            }
        }
    }
