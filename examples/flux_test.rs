    ...
    let output = model
        .eval_pool(
            std::collections::HashMap::from([
                ("latent_sample".to_string(), &latent.view()),
                ("timestep".to_string(), &timestep.view()),
                ("clip_pooled".to_string(), &clip_pooled.view()),
                ("t5_hidden_states".to_string(), &t5_hidden.view()),
            ]),
            &pool,
        )
...
fn test_vae() {
    ...
    let output = model
        .eval_pool(
            std::collections::HashMap::from([("latent_sample".to_string(), &latent.view())]),
            &pool,
        )
