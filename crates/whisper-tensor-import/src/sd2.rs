...
pub fn build_text_encoder(
    ...
    let wm = CastingWeightManager::new(weight_manager.prefix("cond_stage_model.model"), DType::F32);
    let _ = model_dtype;
...
fn openclip_attention(
    ...
    let attn_output = reshape_symbolic(attn_output, vec![0, -1, OPENCLIP_HIDDEN_DIM as i64])?;
...
fn openclip_mlp(wm: &impl WeightManager, input: Arc<dyn Tensor>) -> Result<Arc<dyn Tensor>, Error> {
