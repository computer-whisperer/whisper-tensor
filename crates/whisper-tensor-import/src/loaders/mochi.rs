...
/// Loader for Mochi 1 text-to-video models in HuggingFace Diffusers format.
///
...
pub struct MochiLoader;

impl Loader for MochiLoader {
    fn name(&self) -> &str {
        "Mochi"
    }

    fn description(&self) -> &str {
        "Load Mochi 1 text-to-video model from a HuggingFace Diffusers directory"
    }
    ...
    fn config_schema(&self) -> Vec<ConfigField> {
        vec![ConfigField {
            ...
            description: "Path to the Mochi model directory (e.g. genmo/mochi-1-preview)"
                .to_string(),
    ...
    fn load(&self, config: ConfigValues) -> Result<LoaderOutput, LoaderError> {
        ...
        load_mochi(base_path)
    }
}

fn load_mochi(base_path: PathBuf) -> Result<LoaderOutput, LoaderError> {
    use memmap2::Mmap;
    ...
    println!("Detected Mochi 1 Preview");

    let dit_config = crate::models::diffusion::mochi::MochiTransformerConfig::mochi_preview();
    let vae_config = crate::models::diffusion::mochi::MochiVaeConfig::default_config();
    ...
    let t5_config = crate::models::diffusion::t5::T5Config::t5_xxl(256);
    ...
    println!("Building Mochi DiT transformer...");
    let dit_wm = load_safetensors_dir(&transformer_dir)?;
    let dit_onnx = crate::models::diffusion::mochi::load_mochi_transformer(
        dit_wm, dit_config, storage.clone(),
    )
    .map_err(LoaderError::LoadFailed)?;

    // Build VAE decoder
    println!("Building Mochi VAE decoder...");
    let vae_wm = load_safetensors_dir(&vae_dir)?;
    let vae_onnx = crate::models::diffusion::mochi::load_mochi_vae_decoder(
    ...
    for (suffix, onnx_data) in [
        ...
        models.push(LoadedModel {
            name: format!("mochi-preview-{suffix}"),
            model: Arc::new(model),
        });
    }
    ...
    let interfaces = vec![LoadedInterface {
        name: "mochi-preview-VideoGeneration".to_string(),
...
fn build_mochi_interface(
    ...
    let final_latent = build_mochi_denoising_loop(
        &mut builder, rng,
...
// =============================================================================
// Mochi flow matching denoising loop (Euler step with CFG)
// =============================================================================

#[allow(clippy::too_many_arguments)]
fn build_mochi_denoising_loop(
    ...
    build_progress_init(builder, rng, progress_tier_link, "mochi_progress_init");
    ...
    report.label = Some("mochi_progress".to_string());
    ...
    scan_node.label = Some("mochi_denoise_scan".to_string());
