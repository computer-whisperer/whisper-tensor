...
/// Loader for LTX-Video models in HuggingFace Diffusers format.
///
...
pub struct LtxVideoLoader;

impl Loader for LtxVideoLoader {
    fn name(&self) -> &str {
        "LTX-Video"
    }

    fn description(&self) -> &str {
        "Load LTX-Video text-to-video model from a HuggingFace Diffusers directory"
    }
    ...
    fn config_schema(&self) -> Vec<ConfigField> {
        vec![ConfigField {
            ...
            description: "Path to the LTX-Video model directory (e.g. Lightricks/LTX-Video)"
                .to_string(),
    ...
    fn load(&self, config: ConfigValues) -> Result<LoaderOutput, LoaderError> {
        ...
        load_ltx_video(base_path)
    }
}

fn load_ltx_video(base_path: PathBuf) -> Result<LoaderOutput, LoaderError> {
    use memmap2::Mmap;
    ...
    let num_layers = transformer_config_json["num_layers"]
        .as_u64()
        .unwrap_or(28) as usize;

    let (variant, dit_config) = if num_layers >= 48 {
        (
            "13B",
            crate::models::diffusion::ltx_video::LtxVideoTransformerConfig::ltxv_13b(),
        )
    } else {
        (
            "2B",
            crate::models::diffusion::ltx_video::LtxVideoTransformerConfig::ltxv_2b(),
        )
    };
    println!("Detected LTX-Video-{variant}");

    let vae_config = crate::models::diffusion::ltx_video::LtxVideoVaeConfig::default_config();
    ...
    let t5_config = crate::models::diffusion::t5::T5Config::t5_xxl(128);
    ...
    println!("Building LTX-Video DiT transformer...");
    let dit_wm = load_safetensors_dir(&transformer_dir)?;
    let dit_onnx = crate::models::diffusion::ltx_video::load_ltxv_transformer(
        dit_wm, dit_config, storage.clone(),
    )
    .map_err(LoaderError::LoadFailed)?;

    // Build VAE decoder
    println!("Building LTX-Video VAE decoder...");
    let vae_wm = load_safetensors_dir(&vae_dir)?;
    let vae_onnx = crate::models::diffusion::ltx_video::load_ltxv_vae_decoder(
    ...
    for (suffix, onnx_data) in [
        ...
        models.push(LoadedModel {
            name: format!("ltxv-{}-{suffix}", variant.to_lowercase()),
            model: Arc::new(model),
        });
    }
    ...
    let interfaces = vec![LoadedInterface {
        name: format!("ltxv-{}-VideoGeneration", variant.to_lowercase()),
...
fn build_ltxv_interface(
    ...
    // Denoising loop (flow matching / Euler with CFG)
    let final_latent = build_ltxv_denoising_loop(
        &mut builder, rng,
    ...
    let video_output =
        SuperGraphNodeTensorToVideoClip::new_and_add(&mut builder, decoded_tensor, 24.0, rng);
...
// =============================================================================
// LTX-Video flow matching denoising loop (Euler step with CFG)
// =============================================================================

#[allow(clippy::too_many_arguments)]
fn build_ltxv_denoising_loop(
    ...
    build_progress_init(builder, rng, progress_tier_link, "ltxv_progress_init");
    ...
    report.label = Some("ltxv_progress".to_string());
    ...
    let inner_inputs: Vec<_> = vec![
        ...
        inner_step_in.to_any(), inner_timestep.to_any(), inner_dt.to_any(),
    ];
    ...
    scan_node.label = Some("ltxv_denoise_scan".to_string());
