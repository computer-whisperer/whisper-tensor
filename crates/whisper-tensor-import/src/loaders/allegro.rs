...
/// Loader for Allegro text-to-video models in HuggingFace Diffusers format.
///
/// Expects a directory containing:
/// - `transformer/` — DiT safetensors + config.json
/// - `vae/` — 3D VAE safetensors
/// - `text_encoder/` — T5-XXL safetensors
pub struct AllegroLoader;

impl Loader for AllegroLoader {
    fn name(&self) -> &str {
        "Allegro"
    }

    fn description(&self) -> &str {
        "Load Allegro text-to-video model from a HuggingFace Diffusers directory"
    }
    ...
    fn load(&self, config: ConfigValues) -> Result<LoaderOutput, LoaderError> {
        ...
        load_allegro(base_path)
    }
}

fn load_allegro(base_path: PathBuf) -> Result<LoaderOutput, LoaderError> {
    use memmap2::Mmap;
    ...
    let interfaces = vec![LoadedInterface {
        name: "allegro-VideoGeneration".to_string(),
...
// =============================================================================
// Allegro Euler discrete denoising loop with CFG
// =============================================================================

/// Allegro uses epsilon prediction with EulerAncestral scheduler.
/// CFG with unconditional + conditional DiT passes, Euler step.
/// No Karras sigma scaling (unlike the shared SD-style denoising loop).
#[allow(clippy::too_many_arguments)]
fn build_allegro_denoising_loop(
    ...
    build_progress_init(builder, rng, progress_tier_link, "allegro_progress_init");

    let zero_t5_hidden = builder.new_tensor_link(rng);
    build_zeros_like(builder, rng, t5_hidden, zero_t5_hidden);
    ...
    let inner_zero_t5 = inner_builder.new_tensor_link(rng);
    ...
    // CFG + Euler step (no sigma scaling)
    {
        ...
        let noise_pred = SimpleBinary::add(&mut mg, uncond_f32, scaled, rng);

        let step = SimpleBinary::mul(&mut mg, noise_pred, dt_in, rng);
    ...
    let mut scan_node = SuperGraphNodeScan::new(
        ...
        vec![
            ...
            SuperGraphLinkDouble::new(zero_t5_hidden, inner_zero_t5),
