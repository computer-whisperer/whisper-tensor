...
/// Build a SuperGraph for a KV-cache transformer (processes one token at a time with state).
///
/// `state_pairs` are `(input_name, output_name)` pairs for KV cache tensors.
/// The SuperGraph wraps the model in a Scan loop with RNN cache read/write.
fn build_kv_cache_supergraph(
    tokenizer: TokenizerInfo,
    token_input_name: &str,
    logit_output_name: &str,
