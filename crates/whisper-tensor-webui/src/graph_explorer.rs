...
impl GraphExplorerApp {
    ...
    pub(crate) fn update(
        ...
        if let Some(model_id) = model_id {
            let symbolic_graph =
                if let Some((loaded_id, loaded_graph)) = &mut self.loaded_model_graph {
                        ...
                        minimap_frame.show(ui, |ui| {
                            ...
                            if outer_response.dragged() {
                                ...
                                if let Some(selected_area) =
                                    self.model_view_scene_rects.get_mut(&model_id)
                                {
                                    *selected_area =
                                ...
                                let (is_selected, is_hovered) = match &node.node_type {
                                    ...
                                    | GraphLayoutNodeType::ConnectionByNameDest(link_data) => {
                                        match link_data.link_type {
                                            GraphLayoutLinkType::SymbolicGraphTensor(tensor_id) => {
                                                (
                                                    if let Some(OpOrTensorId::Tensor(
                                                        selected_tensor_id,
                                                    )) = &self.explorer_selection
                                                    {
                                                        *selected_tensor_id == tensor_id
                ...
                frame.show(ui, |ui| {
                    ...
                    scene.show(ui, &mut scene_rect, |ui| {
                        ...
                        for node_id in nodes_to_render_vec {
                            ...
                            let (is_selected, is_hovered) = match &current_node_data[&node_id]
                                .node_type
                            {
                                GraphLayoutNodeType::SymbolicGraphOperation(op_id) => (
                                    if let Some(OpOrTensorId::Op(selected_op_id)) =
                            ...
                            InspectWindow::Operation(op_id) => {
                                ...
                                let resp = window.show(ctx, |ui| {
                                    ...
                                    fn format_tensor_row(
                                        ...
                                        response =
                                            response.union(ui.label(format!("{tensor_id:?}")));
                                        ...
                                        {
                                            new_inspect_windows
                                                .push(OpOrTensorId::Tensor(tensor_id));
                                        }
                                        ui.end_row();
                                        response
                                    }
                            ...
                            InspectWindow::Tensor(inspect_window_tensor) => {
                                ...
                                let resp = window.show(ctx, |ui| {
                                    ...
                                    match &tensor_info.tensor_type {
                                        ...
                                        TensorType::Output => {
                                            resp =
                                                resp.union(ui.label("Tensor Type: Model Output"));
                                        }
                                        TensorType::Intermediate => {
                                            resp =
                                                resp.union(ui.label("Tensor Type: Intermediate"));
                                        }
                                        TensorType::Constant(x) => {
                                            ...
                                            match x {
                                                ...
                                                StoredOrNotTensor::NotStored(x) => {
                                                    resp =
                                                        resp.union(ui.label(format!("Value: {x}")));
                                                }
                                            };
                                        }
                                    }
