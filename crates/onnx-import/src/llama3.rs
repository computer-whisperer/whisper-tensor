...
pub fn load_llama3(weight_manager: impl WeightManager, config: Llama3Config, output_method: WeightStorageStrategy) -> Result<Vec<u8>, anyhow::Error> {
    ...
    let kv_cache_input_shape = Shape::new(vec![batch_dimension,
    ...
    for i in 0..config.num_hidden_layers {
        ...
        let k = linear(&layer_weight_manager.prefix("self_attn.k_proj"), att_norm.clone())?;
        let v = linear(&layer_weight_manager.prefix("self_attn.v_proj"), att_norm)?;

        let q = Transpose::new(None, reshape(q, vec![0, 0, config.num_attention_heads as i64, head_dim as i64])?, Some(vec![0, 2, 1, 3]));
        let k = Transpose::new(None, reshape(k, vec![0, 0, config.num_key_value_heads as i64, head_dim as i64])?, Some(vec![0, 2, 1, 3]));
        ...
        let k = RotaryEmbedding::new(None, k, cos_cache.clone(), sin_cache.clone(), Some(rope_i), None, None, None)?;
        ...
        let scores = div_scalar(
            scores,
            half::bf16::from_f32((head_dim as f32).sqrt()))?;
        ...
        // FeedForward
        let x = linear(&layer_weight_manager.prefix("mlp.gate_proj"), ffn_norm.clone())?;
        let x = silu(x)?;
        let x2 = linear(&layer_weight_manager.prefix("mlp.up_proj"), ffn_norm.clone())?;
