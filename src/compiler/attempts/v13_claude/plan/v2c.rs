...
fn build_group_deps(groups: &[AtomGroup]) -> (Vec<Vec<usize>>, Vec<Vec<usize>>) {
    ...
    for (gi, group) in groups.iter().enumerate() {
        ...
        // 2. Reduce: the strided access extends beyond the InputRef range.
        match &group.op {
            ScalarOp::Reduce {
                reduce_count,
                reduce_stride,
...
fn is_allrows_chain_lane_local_samephase(
    ...
    // Also check Reduce: the reduce itself must be lane-local.
    // For Reduce with reduce_stride != 0, atom i accumulates over
...
fn validate_phase_ordering(
    ...
    for gi in 0..n {
        ...
        // Check 2: Reduce strided access extends the read range.
        match &group.op {
            ScalarOp::Reduce {
                reduce_count,
                reduce_stride,
        ...
        for (phase_idx, phase) in plan.phases.iter().enumerate() {
            ...
            for (lane_idx, lane_work) in phase.lane_work.iter().enumerate() {
                for work in lane_work {
                    ...
                    // For Reduce, also check the strided access.
                    match &group.op {
                        ScalarOp::Reduce {
                            reduce_count,
                            reduce_stride,
