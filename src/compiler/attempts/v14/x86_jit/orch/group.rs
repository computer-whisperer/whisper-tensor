...
fn emit_literal_copy_iter(
    ...
    let (eff_stride, eff_src_base, eff_dst_base) = if byte_fast {
        (
            bit_stride / 8,
            src_store_base_bit / 8,
            dst_store_base_bit / 8,
        )
    } else {
...
fn try_sib_input<'a>(
    ...
    let (base_id, stride_atoms) = match input {
        InputRef::Strided {
            base, dim_strides, ..
        } if dim_strides.len() == 1 => (*base, dim_strides[0]),
        _ => return None,
    };
    ...
    // Runtime-stride slots need a runtime sym_prod multiply the SIB
    // `index*scale+disp` form can't encode. Fall through to the
    // general strided-1d path.
...
fn try_sib_output<'a>(
    ...
    // Runtime-stride output slot: can't bake runtime sym_prod into a
    // SIB `index*scale+disp`. Fall through to the general store path.
    if !slot.sym_dims.is_empty() {
        return None;
    }
