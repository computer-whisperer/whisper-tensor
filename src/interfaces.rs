...
impl TextInferenceTokensInLogitOutInterface {
    ...
    pub fn build_simple_transformer(
    ...
    /// Build a SuperGraph for an RNN-style model (processes one token at a time with state).
    ///
    /// `state_pairs` are `(input_name, output_name)` pairs for recurrent state tensors.
    ...
    pub fn build_rnn(
        ...
        let final_state_outputs: Vec<SuperGraphLinkDouble> = final_state_output_links
            .iter()
            .map(|(id, link)| {
                SuperGraphLinkDouble::Tensor(*state_output_links.get(id).unwrap(), *link)
            })
            .collect();
