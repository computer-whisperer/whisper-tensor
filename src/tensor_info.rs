...
impl<R: Rank> ShapedTensor<R> {
    ...
    pub(crate) fn new_symbolic(
        ...
        for _ in 1..num_values {
            values.push(ScalarInfo::Symbolic(SymbolicScalar::new(dtype, rng)));
        }
        Self {
            dtype,
            shape,
...
where
    T: Clone + PartialEq + Copy + NumericPrimitive,
{
    shape: R::KnownDims,
    ...
    T: Clone + Copy + PartialEq + NumericPrimitive,
{
    ...
    pub(crate) fn new_symbolic(
        ...
        let mut values = vec![first_element];
        for _ in 1..num_values {
            values.push(ScalarInfoTyped::Symbolic(SymbolicScalarTyped::new(rng)));
        }
...
impl<T: Clone + Copy + PartialEq + NumericPrimitive, R: Rank> RankedTensorTyped<T, R> {
...
impl MinimalTensor {
    ...
    pub(crate) fn shape(&self, rng: &mut impl Rng) -> RankedTensorTyped<u64, P1> {
...
/// A concrete typed tensor: all elements are known numeric values.
#[derive(Clone, Debug)]
pub struct ConcreteTypedTensor<T, R: Rank> {
    shape: R::KnownDims,
    values: Vec<T>,
}

impl<T: Clone + Copy + PartialEq, R: Rank> ConcreteTypedTensor<T, R> {
    ...
    fn get(&self, index: &R::KnownDims) -> Option<&T> {
        // Compute flat index from shape + multidim index (row-major).
        ...
        let mut flat = 0usize;
        for (i, (&dim_idx, &dim_size)) in idx.iter().zip(shape.iter()).enumerate() {
            if dim_idx >= dim_size {
                return None;
            }
            flat = flat * dim_size as usize + dim_idx as usize;
            let _ = i;
        }
...
where
    T: Clone + PartialEq + Copy + NumericPrimitive,
{
    Numeric(ConcreteTypedTensor<T, R>),
    ...
    T: Clone + PartialEq + Copy + NumericPrimitive,
{
    pub(crate) fn new_from_scalar_infos(
        ...
        if values.iter().all(|value| value.is_numeric()) {
            let concrete_values: Vec<T> = values
                .into_iter()
                .map(|v| match v {
                    ScalarInfoTyped::Numeric(x) => x,
                    _ => unreachable!(),
                })
                .collect();
            Ok(Self::Numeric(ConcreteTypedTensor {
                shape,
    ...
    T: Clone + PartialEq + Copy + NumericPrimitive,
{
    ...
    pub(crate) fn get(
        ...
        match self {
            ...
            TensorInfoTypedRanked::Ranked(_) => {
                Some(ScalarInfoTyped::Symbolic(SymbolicScalarTyped::new(rng)))
            }
        }
    }
}
    ...
    pub(crate) fn try_to_rank<R1: Rank>(
        ...
        match self {
            TensorInfoShaped::Numeric(x) => {
                ...
                let layout = crate::numeric_tensor::TensorLayout::row_major(new_shape, x.dtype());
...
impl<'a, 'p, R: Rank, P: Pool + 'p> TensorInfoRanked<'a, 'p, R, P> {
    ...
    pub(crate) fn get(&self, index: &R::KnownDims, rng: &mut impl Rng) -> Option<ScalarInfo> {
        match self {
            TensorInfoRanked::Shaped(x) => x.get(index),
            TensorInfoRanked::Ranked(_x) => {
                Some(ScalarInfo::Symbolic(SymbolicScalar::new(self.dtype(), rng)))
            }
        }
    }
        ...
        if let Some(new_shape) = R::try_unknown_to_known_dims(&new_shape) {
            match self {
                TensorInfoRanked::Shaped(x) => {
                    Ok(TensorInfoRanked::Shaped(x.reshape(new_shape, pool)?))
                }
                TensorInfoRanked::Ranked(_x) => {
                    Ok(TensorInfoRanked::Shaped(TensorInfoShaped::Symbolic(
                        ShapedTensor::new_symbolic(self.first_element(), new_shape, rng),
                    )))
                }
            }
        } else {
...
impl<'a, 'p, P: Pool + 'p> TensorInfo<'a, 'p, P> {
    ...
    pub(crate) fn new_from_first_element_and_rank(
        ...
        match rank {
            ScalarInfoTyped::Numeric(x) => {
                ...
                for _ in 0..x {
                    new_dims.push(ScalarInfoTyped::Symbolic(SymbolicScalarTyped::new(rng)));
                }
            ...
            ScalarInfoTyped::Symbolic(x) => {
                TensorInfo::Minimal(MinimalTensor::new(first_element, x))
            }
        }
    }
    ...
    pub(crate) fn new_from_first_element_and_shape(
        ...
        match shape {
            TensorInfoTypedRanked::Shaped(shape) => match shape {
                TensorInfoTypedShaped::Numeric(shape) => {
                    ...
                    TensorInfo::Ranked(TensorInfoRanked::Shaped(TensorInfoShaped::Symbolic(
                        ShapedTensor::<DynRank>::new_symbolic(first_element, shape, rng),
                    )))
                }
            ...
            TensorInfoTypedRanked::Ranked(shape) => {
                Self::new_from_first_element_and_rank(first_element, shape.shape()[0].cast(), rng)
            }
        }
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn shape(&self, rng: &mut impl Rng) -> TensorInfoTypedRanked<u64, P1> {
        match self {
            TensorInfo::Ranked(x) => {
                let shape = x.shape();
    ...
    #[allow(dead_code)]
    pub(crate) fn get(&self, index: &Vec<u64>, rng: &mut impl Rng) -> Option<ScalarInfo> {
    ...
    pub fn from_dtype_and_shape_scalars(
        dtype: NumericDType,
        shape: &[ScalarInfoTyped<u64>],
    ) -> Self {
        ...
        match self {
            ...
            TensorInfo::Minimal(tensor) => {
                ...
                for _ in 0..R::KNOWN_LEN {
                    new_shape.push(ScalarInfoTyped::Symbolic(SymbolicScalarTyped::new(rng)));
                }
