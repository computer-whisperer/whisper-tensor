...
fn build_matmul_pipeline(
    ...
    builder.decorate(
        ...
        [Operand::LiteralBit32(
            stored_dtype.bytes_per_element().unwrap() as u32,
        )],
    );
...
impl<R: Rank> VulkanTensor<R> {
    pub fn matmul(
        ...
        let op_metadata_vec = [
            a.suballocation.offset as u32 / a.dtype().bytes_per_element().unwrap() as u32
                + a.offset as u32,
            b.suballocation.offset as u32 / b.dtype().bytes_per_element().unwrap() as u32
                + b.offset as u32,
