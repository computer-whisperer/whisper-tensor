...
pub trait NDArrayNumericTensorType: Sized + DTypeOfPrimitive + Clone {
    fn ndarray_numeric_tensor_from_vec_shape<R: Rank>(
        v: Vec<Self>,
        shape: R::NDArrayDim,
    ) -> Result<NDArrayNumericTensor<R>, NDArrayNumericTensorError>;
    fn ndarray_numeric_tensor_from_vec_shape_stride<R: Rank>(
        v: Vec<Self>,
        shape: R::NDArrayDim,
        stride: R::NDArrayDim,
    ) -> Result<NDArrayNumericTensor<R>, NDArrayNumericTensorError>;
    fn ndarray_numeric_tensor_from_vec(v: Vec<Self>) -> NDArrayNumericTensor<P1>;
    fn ndarray_numeric_tensor_ref_parts<R: Rank>(
        tensor: &NDArrayNumericTensor<R>,
    ) -> Result<(&[Self], &[usize]), NDArrayNumericTensorError>;
    fn ndarray_numeric_tensor_inner<R: Rank>(
        tensor: &NDArrayNumericTensor<R>,
    ) -> Result<&ArcArray<Self, R::NDArrayDim>, NDArrayNumericTensorError>;
    fn ndarray_numeric_tensor_from_ndarray<R: Rank>(
        value: ArcArray<Self, R::NDArrayDim>,
    ) -> NDArrayNumericTensor<R>;
}
...
impl<R: Rank> NDArrayNumericTensor<R> {
    ...
    pub(crate) fn from_slice_shape_stride<T: NDArrayNumericTensorType>(
        v: Vec<T>,
        shape: &R::KnownDims,
        stride: &R::KnownDims,
    ) -> Result<Self, NDArrayNumericTensorError> {
        T::ndarray_numeric_tensor_from_vec_shape_stride(
            v,
            R::cast_to_ndarray_dim(shape),
            R::cast_to_ndarray_dim(stride),
        )
    }
}

impl<R: Rank> NDArrayNumericTensor<R> {
    pub(crate) fn try_from_vec_shape<T: NDArrayNumericTensorType>(
        v: Vec<T>,
        shape: &[usize],
    ) -> Result<Self, NDArrayNumericTensorError> {
...
impl NDArrayNumericTensor<P1> {
    ...
    pub fn try_to_vec<T: NDArrayNumericTensorType>(
        &self,
    ) -> Result<Vec<T>, NDArrayNumericTensorError> {
...
impl<R: Rank> NDArrayNumericTensor<R> {
    pub fn as_inner<T: NDArrayNumericTensorType>(
        &self,
    ) -> Result<&ArcArray<T, R::NDArrayDim>, NDArrayNumericTensorError> {
        ...
        impl NDArrayNumericTensorType for $a {
            ...
            fn ndarray_numeric_tensor_ref_parts<R: Rank>(
                ...
                if let NDArrayNumericTensor::$b(x) = tensor {
                    ...
                    Err(NDArrayNumericTensorError::WrongDTypeError(
                        Self::DTYPE,
                        tensor.dtype(),
                    ))
                }
...
impl_type_ndarray_backend!(F8E4M3, F8E4M3FN);
...
impl_type_ndarray_backend!(u4, U4);
impl_type_ndarray_backend!(i4, I4);
