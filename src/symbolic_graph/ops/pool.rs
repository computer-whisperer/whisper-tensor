...
impl MaxPoolOperation {
    pub(crate) fn from_onnx(
        ...
        if inputs.len() != 1 {
            return Err(ONNXDecodingError::InvalidOperatorInputs("MaxPool"));
        }
        if outputs.is_empty() {
            return Err(ONNXDecodingError::InvalidOperatorOutputs("MaxPool"));
        }
        ...
        Ok(Self {
            global_id: GlobalId::new(rng),
            input: inputs[0].ok_or(ONNXDecodingError::InvalidOperatorInputs("MaxPool"))?,
            output: outputs[0].ok_or(ONNXDecodingError::InvalidOperatorOutputs("MaxPool"))?,
            kernel_shape,
            strides,
            pads,
            dilations,
...
impl Node for MaxPoolOperation {
    type OpKind = String;
    fn global_id(&self) -> GlobalId {
        self.global_id
    }
...
impl Operation for MaxPoolOperation {
    fn parameters(&self) -> Vec<Property> {
        let mut params = Vec::new();
        let auto_pad_str = match &self.auto_pad {
            PoolAutoPad::NotSet => "NOTSET",
    ...
    fn get_milli_op_graph(&self, _ctx: &MilliLoweringContext, rng: &mut impl Rng) -> MilliOpGraph {
        ...
        for k_idx in 0..kernel_size {
            ...
            max_acc = Some(match max_acc {
                None => tap,
                Some(acc) => milli_ops::SimpleBinary::max(&mut graph, acc, tap, rng),
            });
        }

        let result = max_acc.unwrap();
...
impl LpPoolOperation {
    pub(crate) fn from_onnx(
        ...
        if inputs.len() != 1 {
            return Err(ONNXDecodingError::InvalidOperatorInputs("LpPool"));
        }
        if outputs.is_empty() {
            return Err(ONNXDecodingError::InvalidOperatorOutputs("LpPool"));
        }
        ...
        let dilations = query_attribute_ints(attributes, "dilations").unwrap_or_default();
        let ceil_mode = query_attribute_int(attributes, "ceil_mode").unwrap_or(0) != 0;
        let p = query_attribute_int(attributes, "p").unwrap_or(2) as f32;

        Ok(Self {
            global_id: GlobalId::new(rng),
            input: inputs[0].ok_or(ONNXDecodingError::InvalidOperatorInputs("LpPool"))?,
            output: outputs[0].ok_or(ONNXDecodingError::InvalidOperatorOutputs("LpPool"))?,
            kernel_shape,
            strides,
            pads,
            dilations,
...
impl Operation for LpPoolOperation {
    fn parameters(&self) -> Vec<Property> {
        let mut params = Vec::new();
        let auto_pad_str = match &self.auto_pad {
            PoolAutoPad::NotSet => "NOTSET",
        ...
        params.push(Property::new("p", PropertyValue::Float(self.p.into())));
        params
    }
    ...
    fn get_milli_op_graph(&self, _ctx: &MilliLoweringContext, rng: &mut impl Rng) -> MilliOpGraph {
        ...
        // Compute padding (same logic as AveragePool but pad with 0)
        let pads_const: Option<GlobalId> = match &self.auto_pad {
            PoolAutoPad::Valid => None,
            PoolAutoPad::NotSet => {
                if self.pads.is_empty() || self.pads.iter().all(|&p| p == 0) {
        ...
        // Pad with 0 (zero contributes nothing to |x|^p sum)
        let padded = if let Some(pc) = pads_const {
            let zero_val = ops_helpers::scalar_const(&mut graph, 0.0f32, rng);
            milli_ops::Pad::push_new(
                &mut graph,
        ...
        // Apply |x|^p to the padded tensor
        let p_const = ops_helpers::scalar_const(&mut graph, self.p, rng);
        let inv_p_const = ops_helpers::scalar_const(&mut graph, 1.0f32 / self.p, rng);
        let abs_padded = milli_ops::SimpleUnaryOp::abs(&mut graph, padded, rng);
        let powered = milli_ops::Pow::push_new(&mut graph, abs_padded, p_const, rng);
        ...
        // result = sum^(1/p)
        let result = milli_ops::Pow::push_new(&mut graph, sum_acc.unwrap(), inv_p_const, rng);
