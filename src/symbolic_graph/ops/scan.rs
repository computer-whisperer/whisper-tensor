    ...
    fn eval_pool<'p, P: crate::pool::Pool + 'p>(
        ...
        fn slice_and_squeeze<'p2, P2: crate::pool::Pool + 'p2>(
            ...
            for flat_out in 0..out_numel {
                ...
                let _remaining = flat_out;
                let _src_flat = index as usize * strides[axis];
                let dim_idx = 0;
                for i in 0..rank {
                    if i == axis {
                        continue;
        ...
        fn concat_along<'p2, P2: crate::pool::Pool + 'p2>(
            ...
            let _out_numel: usize = out_shape.iter().product::<u64>() as usize;
        ...
        for iter_idx in 0..iter_count {
            ...
            let mut input_idx = 0;
            for t in state_tensors.iter().flatten() {
                let view = t.view();
                let layout =
                    TensorLayout::<DynRank>::row_major(view.shape().to_vec(), view.dtype());
                let buf = pool
