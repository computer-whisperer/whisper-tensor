...
pub(crate) fn rank(graph: &mut MilliOpGraph<TensorId>, tensor: MilliOpGraphTensorId) -> MilliOpGraphTensorId {
...
pub(crate) fn resolve_axes(graph: &mut MilliOpGraph<TensorId>, axes: MilliOpGraphTensorId, tensor: MilliOpGraphTensorId) -> MilliOpGraphTensorId {
    let shape = graph.push_op(AnyMilliOp::Shape(MilliOpShape::new(tensor)));
