...
impl MilliOp for ReduceSum {
    ...
    > {
        ...
        // Propagate dtype (same as input) and infer output rank.
        ...
        let num_axes: Option<usize> = if let Some(ax_id) = self.axes {
            if let Some(ax_info) = known_inputs.get(&ax_id) {
                ax_info.rank_if_known().and_then(|_| {
                    // axes is rank 1; its length is the number of axes
            ...
            // No axes tensor: reduce all axes
            None
        };
        ...
        let out_rank: ScalarInfoTyped<u32> = match data_info.rank() {
            ScalarInfoTyped::Numeric(input_rank) => {
                if self.axes.is_none() {
                    ...
                    // keepdims preserves rank regardless of which axes
                    ScalarInfoTyped::Numeric(input_rank)
                } else {
                    // Don't know how many axes are reduced
