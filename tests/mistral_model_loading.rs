...
fn find_mistral_dir() -> Option<PathBuf> {
    let candidates = [
        "/ceph/public/neural_models/llms/Mistral-7B-Instruct-v0.2",
        "/ceph/public/neural_models/llms/mistralai_Mistral-7B-v0.1",
        "/ceph/public/neural_models/llms/Mistral-Small-24B-Instruct-2501",
    ];
    for c in candidates {
        let p = Path::new(c);
...
/// Test that a Mistral ONNX graph builds successfully from safetensors weights.
#[test]
fn mistral_graph_builds() {
    let Some(model_dir) = find_mistral_dir() else {
        eprintln!("Skipping: no Mistral model directory found");
        return;
    };
...
/// Test that the TransformersLoader produces a working interface for Mistral.
#[test]
fn mistral_loads_via_transformers_loader() {
    let Some(model_dir) = find_mistral_dir() else {
        eprintln!("Skipping: no Mistral model directory found");
        return;
    };
