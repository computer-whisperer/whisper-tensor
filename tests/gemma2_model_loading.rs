...
fn find_gemma2_dir() -> Option<PathBuf> {
    let candidates = [
        "/ceph/public/neural_models/llms/gemma-2-2b-it",
        "/ceph/public/neural_models/llms/gemma-2-9b-it",
        "/ceph/public/neural_models/llms/gemma-2-27b-it",
    ];
    for c in candidates {
        let p = Path::new(c);
...
/// Test that a Gemma2 ONNX graph builds successfully from safetensors weights.
#[test]
fn gemma2_graph_builds() {
    let Some(model_dir) = find_gemma2_dir() else {
        eprintln!("Skipping: no Gemma2 model directory found");
        return;
    };
...
/// Test that the TransformersLoader produces a working interface for Gemma2.
#[test]
fn gemma2_loads_via_transformers_loader() {
    let Some(model_dir) = find_gemma2_dir() else {
        eprintln!("Skipping: no Gemma2 model directory found");
        return;
    };
