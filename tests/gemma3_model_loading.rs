...
fn find_gemma3_dir() -> Option<PathBuf> {
    let candidates = [
        "/ceph/public/neural_models/llms/gemma-3-1b-it",
        "/ceph/public/neural_models/llms/gemma-3-4b-it",
        "/ceph/public/neural_models/llms/gemma-3-12b-it",
        "/ceph/public/neural_models/llms/gemma-3-27b-it",
    ];
    for c in candidates {
        let p = Path::new(c);
...
fn num_text_layers(config: &serde_json::Value) -> usize {
    if config["model_type"].as_str() == Some("gemma3") {
        config["text_config"]["num_hidden_layers"].as_u64().unwrap() as usize
    } else {
        config["num_hidden_layers"].as_u64().unwrap() as usize
    }
}

/// Test that a Gemma3 ONNX graph builds successfully from safetensors weights.
#[test]
fn gemma3_graph_builds() {
    let Some(model_dir) = find_gemma3_dir() else {
        eprintln!("Skipping: no Gemma3 model directory found");
        return;
    };

    let config_str = std::fs::read_to_string(model_dir.join("config.json")).unwrap();
    let config: serde_json::Value = serde_json::from_str(&config_str).unwrap();
    let num_layers = num_text_layers(&config);
...
/// Test that the TransformersLoader produces a working interface for Gemma3.
#[test]
fn gemma3_loads_via_transformers_loader() {
    let Some(model_dir) = find_gemma3_dir() else {
        eprintln!("Skipping: no Gemma3 model directory found");
        return;
    };
