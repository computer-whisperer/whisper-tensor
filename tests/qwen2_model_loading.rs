...
fn find_qwen2_dir() -> Option<PathBuf> {
    let candidates = [
        "/ceph/public/neural_models/llms/Qwen2-0.5B",
        "/ceph/public/neural_models/llms/Qwen2.5-Coder-3B-Instruct",
    ];
    for c in candidates {
        let p = Path::new(c);
...
/// Test that the Qwen2 ONNX graph builds successfully from safetensors weights.
#[test]
fn qwen2_graph_builds() {
    let Some(model_dir) = find_qwen2_dir() else {
        eprintln!("Skipping: no Qwen2 model directory found");
        return;
    };
    ...
    eprintln!(
        "Testing with {} ({} layers, tie_embeddings={})",
        model_dir.display(),
        num_layers,
        tie_word_embeddings
    );
...
/// Test that the TransformersLoader produces a working interface for Qwen2.
#[test]
fn qwen2_loads_via_transformers_loader() {
    let Some(model_dir) = find_qwen2_dir() else {
        eprintln!("Skipping: no Qwen2 model directory found");
        return;
    };
